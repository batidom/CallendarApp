...
const DATE_TIME_FIELDS = {
  day: { type: 'string', description: 'Date in YYYY-MM-DD, in the user\'s local time.' },
  startTime: {
    type: 'string',
    description: '24-hour HH:mm local time. Omit for an all-day item or one with no specific time.',
  },
  endTime: { type: 'string', description: '24-hour HH:mm local time. Defaults to one hour after startTime.' },
  isAllDay: { type: 'boolean' },
  repeat: {
    type: 'string',
    enum: ['none', 'daily', 'weekly', 'monthly', 'yearly'],
    description: 'How the event repeats. "none" (or omitted) means it does not repeat.',
  },
  repeatInterval: {
    type: 'integer',
  ...
  repeatUntil: { type: 'string', description: 'YYYY-MM-DD the repeat stops on. Omit for no end date.' },
  reminderMinutesBefore: {
    type: 'array',
    items: { type: 'integer' },
    description: 'Minutes before start to remind, e.g. [10, 1440] for 10 minutes and 1 day before.',
  },
} as const;
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          titleContains: {
            type: 'string',
            description: 'Case-insensitive substring to match against titles. Omit to match any title.',
          },
          dateFrom: { type: 'string', description: 'YYYY-MM-DD, inclusive start of range.' },
          dateTo: { type: 'string', description: 'YYYY-MM-DD, inclusive end of range.' },
        },
      },
    },
  },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          confirmNotDuplicate: {
            type: 'boolean',
            description: 'Only set true after a prior create_event call for this same title was rejected as a possible duplicate, and you have confirmed it is really a different event.',
          },
          confirmOverlap: {
            type: 'boolean',
            description: 'Only set true after a prior call was rejected for overlapping an existing event, and the user has confirmed they want to schedule it anyway despite the conflict.',
          },
          ...DATE_TIME_FIELDS,
        },
        required: ['title'],
